...
include { softmasking } from './modules/softmasking.nf'
...
/////////////// TRANSCRIPTOME QC PIPELINE ///////////////
  ...
  busco(params.transcripts, Channel.fromList(params.busco_lineages), "transcriptome")
...
// nextflow run palfalvi/nanoporeseq --mode annotation --genome $genome --skip_softmask --short_reads $short_reads --ont_reads $ont_reads
  ...
  if ( params.short_reads != false ) {
    ...
    taco_stringtie2_short.out.gtf
      .mix( taco_strawberry_short.out.gtf, taco_trinity_short.out.gtf )
      .set { short_gtf }
